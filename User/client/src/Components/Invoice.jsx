...
const Invoice = ({ invoice, onClose, orgData, invoiceSettings, autoDownload = false }) => {
  ...
  const isExtension = (() => {
    // Storage extension: cumulative MB stored on the invoice differs from the plan's MB.
    if (
    ...
    // Date extension: invoice expiry differs from "fresh" (invoiceDate + days - 1).
    if (
  ...
  // Backend sends LocalDate as "YYYY-MM-DD"; render DD/MM/YYYY to match the rest of the invoice.
  ...
  const [invoiceData, setInvoiceData] = useState({
    ...
    // Prefer the snapshot fields persisted on the invoice (set by the backend
    // at generation time). Fall back to user fields for invoices created before
    // the snapshot existed — keeps old PDFs from going blank after this change.
