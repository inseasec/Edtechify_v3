...
} from "../../constants/homePageLiveDefaults.js";
import HomeTwoPortalsOffering from "../HomeTwoPortalsOffering.jsx";
...
/** Slide ids + built-in fallbacks (CRM overrides per slide in HomeBannerCarousel). */
const BANNER_SLIDES = [
  { id: "portal", headline: HOME_HERO_TITLE_LIVE_DEFAULT },
  { id: "web", headline: HOME_HERO_SUBTITLE_LIVE_DEFAULT },
  { id: "subscribe", headline: HOME_TRUST_STRIP_LIVE_DEFAULT },
];
...
function CtaButton({ to, className, children, previewMode }) {
  if (previewMode) {
    return (
      <span className={`${className} inline-block cursor-default opacity-95`} title="Preview — link disabled in admin">
        {children}
      </span>
    );
  }
...
function InsightLink({ to, className, children, previewMode }) {
  if (previewMode) {
...
function SolutionTile({ tile, previewMode }) {
  const inner = (
    <>
      <span className="text-lg font-semibold leading-snug tracking-tight text-slate-900 group-hover:text-sky-900">
        {tile.title}
      </span>
      <span className="mt-3 flex-1 text-sm leading-relaxed text-slate-600">{tile.teaser}</span>
      <span className="mt-4 text-sm font-semibold text-sky-700 group-hover:underline">Read more</span>
    </>
  );

  const cls =
    "group flex h-full min-h-[9.5rem] flex-col rounded-2xl border border-white/10 border-l-4 border-l-sky-500 bg-white p-6 shadow-xl shadow-black/35 transition hover:border-sky-400/50 hover:shadow-2xl sm:min-h-[10rem] sm:p-7";

  if (previewMode) {
    return (
      <div className={`${cls} cursor-default`} title="Preview">
        {inner}
      </div>
    );
  }
  ...
  return (
    <Link to={`/solution/${tile.slug}`} className={cls}>
      {inner}
    </Link>
  );
}

function HomeBannerCarousel({ bannerDisplay, orgHome, previewMode }) {
  const [index, setIndex] = useState(0);
  const [paused, setPaused] = useState(false);
  const n = BANNER_SLIDES.length;

  const portalHeadline =
    typeof orgHome?.homeHeroTitle === "string" && orgHome.homeHeroTitle.trim()
      ? orgHome.homeHeroTitle.trim()
      : HOME_HERO_TITLE_LIVE_DEFAULT;

  const webHeadline =
    typeof orgHome?.homeHeroSubtitle === "string" && orgHome.homeHeroSubtitle.trim()
      ? orgHome.homeHeroSubtitle.trim()
      : HOME_HERO_SUBTITLE_LIVE_DEFAULT;

  const subscribeHeadline =
    typeof orgHome?.homeTrustStrip === "string" && orgHome.homeTrustStrip.trim()
      ? orgHome.homeTrustStrip.trim()
      : HOME_TRUST_STRIP_LIVE_DEFAULT;
  ...
  const headlineForSlide =
    slide.id === "portal" ? portalHeadline : slide.id === "web" ? webHeadline : subscribeHeadline;
    ...
    >
      <div className="relative mx-auto w-full max-w-[104.544rem] overflow-hidden">
        <div className="relative min-h-[min(72vh,640px)] w-full md:min-h-[min(78vh,720px)]">
          {/* One backdrop layer: video/img never remounts when arrows change slides */}
          <div className="absolute inset-0 z-0 overflow-hidden" aria-hidden>
            {bannerDisplay ? (
              ...
              <BannerArtBackdrop variant="portal" priority />
            )}
          </div>

          <div
            className="pointer-events-none absolute inset-x-0 bottom-0 z-[1] h-36 bg-gradient-to-t from-slate-950/75 via-slate-950/35 to-transparent sm:h-40"
            aria-hidden
          />

          <div className="relative z-[2] flex h-full min-h-[inherit] flex-col justify-end pb-20 pt-28 sm:justify-center sm:pb-24 sm:pt-20 md:pb-28">
            <div className="mx-auto w-full max-w-[104.544rem] px-4 sm:px-6 lg:px-8">
              <div className="max-w-xl rounded-2xl border border-white/25 bg-slate-950/45 p-6 shadow-2xl shadow-black/20 backdrop-blur-md sm:max-w-2xl sm:p-8 md:max-w-3xl md:bg-slate-950/50 lg:max-w-[44rem]">
                <div key={index} className="home-hero-copy-enter">
                  {headlineForSlide.includes("—") ? (
                    <h2 className="mt-4 max-w-2xl text-balance font-serif text-[clamp(1.75rem,1rem+3.4vw,3rem)] font-semibold leading-[1.15] tracking-[-0.02em] text-white lg:max-w-3xl">
                      {(() => {
                        const [lead, tail] = headlineForSlide.split("—");
                        return (
                          <>
                ...
                <div className="mt-8 flex flex-wrap gap-3 sm:mt-9">
                  <CtaButton to="/signup" className={primaryCta} previewMode={previewMode}>
                    Get started
                  </CtaButton>
                  <CtaButton to="/contact" className={secondaryCta} previewMode={previewMode}>
                    Contact us
                  </CtaButton>
                </div>
              </div>
...
/**
 * Full marketing home layout — used on the public site and in admin preview (`previewMode`).
 */
  ...
  return (
    <div className="w-full bg-slate-50 text-[var(--text)]">
      ...
      <HomeTwoPortalsOffering
        previewMode={previewMode}
        offeringsIntro={
          typeof orgHome?.homeOfferingsIntro === "string" ? orgHome.homeOfferingsIntro : ""
        }
      />
          ...
          <div className="relative z-[1]">
            ...
            <ul className="mt-10 grid list-none gap-6 sm:grid-cols-2 sm:gap-8">
              {solutionInsightTiles.map((tile) => (
                <li key={tile.slug}>
                  <SolutionTile tile={tile} previewMode={previewMode} />
                </li>
              ))}
            </ul>
          </div>
